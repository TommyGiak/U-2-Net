...
for EVAL_MODEL_N in range(-1,6):

  models_numbers_tmp = models_numbers.copy()
  if EVAL_MODEL_N >=0:
    models_numbers_tmp.remove(EVAL_MODEL_N)

  test_img_list = []
  test_lbl_list = []

  for i in models_numbers_tmp:
  ...
  if EVAL_MODEL_N>=0:
    ...
    os.makedirs(f'metrics/model_synth{EVAL_MODEL_N}/', exist_ok=True)
    pd.DataFrame(wound_metrics).to_csv(f'metrics/model_synth{EVAL_MODEL_N}/wound.csv')
    pd.DataFrame(body_wound_marker_metrics).to_csv(f'metrics/model_synth{EVAL_MODEL_N}/body_wound_marker.csv')
    pd.DataFrame(marker_metrics).to_csv(f'metrics/model_synth{EVAL_MODEL_N}/marker.csv')
    pd.DataFrame(body_marker_no_wound_metrics).to_csv(f'metrics/model_synth{EVAL_MODEL_N}/body_marker_no_wound.csv')
    pd.DataFrame(body_only_metrics).to_csv(f'metrics/model_synth{EVAL_MODEL_N}/body_only.csv')
  else:
    os.makedirs('metrics/', exist_ok=True)
    os.makedirs(f'metrics/model_synth_full/', exist_ok=True)
    pd.DataFrame(wound_metrics).to_csv(f'metrics/model_synth_full/wound.csv')
    pd.DataFrame(body_wound_marker_metrics).to_csv(f'metrics/model_synth_full/body_wound_marker.csv')
    pd.DataFrame(marker_metrics).to_csv(f'metrics/model_synth_full/marker.csv')
    pd.DataFrame(body_marker_no_wound_metrics).to_csv(f'metrics/model_synth_full/body_marker_no_wound.csv')
    pd.DataFrame(body_only_metrics).to_csv(f'metrics/model_synth_full/body_only.csv')
